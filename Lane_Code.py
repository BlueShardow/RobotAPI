#This code sends to a custom web socket server, not the gui
...
def detect_and_classify_lines(frame, max_line_gap=85, toward_tolerance = 65, away_tolerance = 45, merge_angle_tolerance=65,
    ...
    lower_blue = np.array([112, 70, 105])  # Widened lower bound for blue
    upper_blue = np.array([290, 255, 255])  # Upper bound for blue
...
while True:
    ...
    # Perform line detection and classification
    detected_frame, blue_mask, blue_mask_dilated = detect_and_classify_lines(frame.copy())

    # Display the result
    cv2.imshow('Line Classification', detected_frame)
    cv2.imshow('Blue Detection', blue_mask)
    cv2.imshow('Blue Proximity Detection', blue_mask_dilated)
...
"""DEBUGGING ABOVE________________________"""

# Define the global video capture object
cap = cv2.VideoCapture(0)
if not cap.isOpened():
    print("Error: Unable to access the camera.")
    exit()

async def send_plain_video(websocket):
    """
    Handles the plain video WebSocket connection.
    """
    print(f"Plain WebSocket client connected")
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
            ...
            print("Sent plain frame")
            await asyncio.sleep(0.03)
    except Exception as e:
        print(f"Error in plain video stream: {e}")
    finally:
        print("Plain WebSocket client disconnected")

async def send_overlay_video(websocket):
    """
    Handles the overlay video WebSocket connection.
    """
    print(f"Overlay WebSocket client connected")
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
            ...
            frame_data = base64.b64encode(buffer).decode('utf-8')
            await websocket.send(frame_data)
            print("Sent overlay frame")
            await asyncio.sleep(0.03)
    except Exception as e:
        print(f"Error in overlay video stream: {e}")
    finally:
        print("Overlay WebSocket client disconnected")

async def main():
    """
    Starts the WebSocket servers for plain and overlayed video streams.
    """
    plain_server = websockets.serve(send_plain_video, "localhost", 8765)
    overlay_server = websockets.serve(send_overlay_video, "localhost", 8766)

    print("Starting WebSocket servers:")
    print(" - Plain video: ws://localhost:8765")
    print(" - Overlay video: ws://localhost:8766")

    # Start the servers and keep the loop running
    await asyncio.gather(
        plain_server,
        overlay_server,
        asyncio.Future()  # Keeps the event loop running forever
    )

"""For debugging, comment out if not__________________"""
...
# Debug detect_and_classify_lines
processed_frame, blue_mask, blue_mask_dilated = detect_and_classify_lines(frame.copy())
if processed_frame is None or blue_mask is None or blue_mask_dilated is None:
    print("Error: Processing returned None frames.")
    cap.release()
    cv2.destroyAllWindows()
    exit()

print("Processed Frame Shape:", processed_frame.shape)
print("Blue Mask Unique Values:", np.unique(blue_mask))
print("Blue Mask Dilated Unique Values:", np.unique(blue_mask_dilated))

cv2.imshow("Overlay Feed", processed_frame)
cv2.imshow("Blue Mask Feed", blue_mask)
cv2.imshow("Blue Mask Proximity Feed", blue_mask_dilated)
cv2.waitKey(500)  # Add delay for visibility
cv2.destroyAllWindows()

#______________________________________________________________________________________________

# Initialize the video feed and process a single frame for debugging
ret, frame = cap.read()
if not ret:
...
# Process the initial frame for debugging
processed_frame, blue_mask, blue_mask_dilated = detect_and_classify_lines(frame.copy())

while True:
    # Display debugging feeds
    cv2.imshow("Overlay Feed", processed_frame)
    cv2.imshow("Blue Mask Feed", blue_mask)
    cv2.imshow("Blue Mask Proximity Feed", blue_mask_dilated)

    # Handle OpenCV window events
    if cv2.waitKey(1) & 0xFF == ord('q'):  # Press 'q' to quit the local display
        break
"""
